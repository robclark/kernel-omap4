...
uint8_t *omaprpc_map_parameter(struct omaprpc_instance_t *rpc, struct omaprpc_param_t *param)
{
    ...
    bkva = (uint8_t *)ion_map_kernel(rpc->ion_client, (struct ion_handle *)param->reserved);

    /* set the kernel VA equal to the base kernel VA plus the primary offset */
    kva = &bkva[pri_offset];
...
void omaprpc_unmap_parameter(struct omaprpc_instance_t *rpc, struct omaprpc_param_t *param, uint8_t *ptr, uint32_t sec_offset)
{
...
phys_addr_t omaprpc_buffer_lookup(struct omaprpc_instance_t *rpc, uint32_t core, virt_addr_t uva, virt_addr_t buva, void *reserved)
{
    ...
    if (uoff < 0)
    {
        OMAPRPC_ERR(rpc->rpcserv->dev, "Offsets calculation for BUVA=%p from UVA=%p is a negative number. Bad parameters!\n", (void *)buva, (void *)uva);
        rpa = 0; // == NULL
        goto to_end;
    }
...
restart:
    for (idx = start; idx != limit; idx+=inc)
    {
        ...
        {
            OMAPRPC_ERR(rpc->rpcserv->dev, "Invalid parameter pointer index %u\n", ptr_idx);
            goto unwind;
        }
        else if (function->params[ptr_idx].type != OMAPRPC_PARAM_TYPE_PTR)
        {
            OMAPRPC_ERR(rpc->rpcserv->dev, "Parameter index %u is not a pointer (type %u)\n", ptr_idx, function->params[ptr_idx].type);
            goto unwind;
        }
        ...
        {
            OMAPRPC_ERR(rpc->rpcserv->dev, "Offset is larger than data area! (offset=%u size=%u)\n", offset, size);
            goto unwind;
        }

        if (function->params[ptr_idx].data == 0)
        {
            OMAPRPC_ERR(rpc->rpcserv->dev, "Supplied user pointer is NULL!\n");
            goto unwind;
        }
        ...
        {
            /* map the UVA pointer to KVA space, the offset could potentially be modified due to the mapping */
            base_ptrs[ptr_idx] = omaprpc_map_parameter(rpc, &function->params[ptr_idx]);
        }
        ...
        if (base_ptrs[ptr_idx] != NULL)
        {
            if (direction == OMAPRPC_UVA_TO_RPA)
            {
                ...
                if ((kva & 0x3) > 0) {
                    OMAPRPC_ERR(rpc->rpcserv->dev, "ERROR: KVA %p is unaligned!\n",(void *)kva);
                ...
                *(phys_addr_t *)kva = rpa;
                ...
                OMAPRPC_INFO(rpc->rpcserv->dev, "Replaced UVA %p with RPA %p at KVA %p\n", (void *)uva, (void *)rpa, (void *)kva);
            ...
            else if (direction == OMAPRPC_RPA_TO_UVA)
            {
                ...
                *(virt_addr_t *)kva = uva;
                ...
                OMAPRPC_INFO(rpc->rpcserv->dev, "Replaced RPA %p with UVA %p at KVA %p\n", (void *)rpa, (void *)uva, (void *)kva);
        ...
        {
            OMAPRPC_ERR(rpc->rpcserv->dev, "Failed to map UVA to KVA to do translation!\n");
...
unwind:
            if (direction == OMAPRPC_UVA_TO_RPA)
            {
                ...
                OMAPRPC_ERR(rpc->rpcserv->dev, "Unwinding UVA to RPA translations!\n");
                direction = OMAPRPC_RPA_TO_UVA;
                start = idx-1;
                inc = -1;
                limit = -1;
    ...
    {
        if (base_ptrs[idx]) {
            omaprpc_unmap_parameter(rpc, &function->params[idx], base_ptrs[idx], 0);
            base_ptrs[idx] = NULL;
        }
    }
    return ret;
