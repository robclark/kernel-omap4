...
static dma_info_t *omaprpc_dma_sub(struct omaprpc_instance_t *rpc, int fd)
{
    dma_info_t *pos, *n;
    mutex_lock(&rpc->lock);
...
static int omaprpc_dma_add(struct omaprpc_instance_t *rpc, dma_info_t *dma)
{
    if (dma)
    {
        mutex_lock(&rpc->lock);
...
{
    dma_info_t *dma = (dma_info_t *)kmalloc(sizeof(dma_info_t), GFP_KERNEL);
    if (dma)
    {
        dma->fd = (int)reserved;
...
{
    dma_info_t *dma = omaprpc_dma_sub(rpc, (int)reserved);
    if (dma)
    {
...
int omaprpc_xlate_buffers(struct omaprpc_instance_t *rpc, struct omaprpc_call_function_t *function, int direction)
{
    ...
    uint32_t ptr_idx = 0, pri_offset = 0, sec_offset = 0, pg_offset = 0, size = 0;
    ...
    for (idx = start; idx != limit; idx+=inc)
    {
        ...
        {
            OMAPRPC_ERR(rpc->rpcserv->dev, "Offset is larger than data area! (sec_offset=%u size=%u)\n", sec_offset, size);
            goto unwind;
        }
        ...
        if (base_ptrs[ptr_idx] != NULL)
        {
            if (direction == OMAPRPC_UVA_TO_RPA)
            {
                ...
                virt_addr_t kva = (virt_addr_t)&((base_ptrs[ptr_idx])[pg_offset]);
                virt_addr_t uva = 0;
                ...
                *(phys_addr_t *)kva = rpa;
                ...
                if (rpa == 0) {
                    ...
                    limit = -1;
                    ret = -ENODATA;
                    // @TODO unmap the parameter base pointer
                    goto restart;
                }
            }
            else if (direction == OMAPRPC_RPA_TO_UVA)
            {
                ...
                *(virt_addr_t *)kva = uva;
                ...
                // @TODO DMA_BUF requires unmapping the data from the TILER.
...
restart:
        if (base_ptrs[ptr_idx]) {
            ...
            dma_buf_end_cpu_access(dbufs[ptr_idx], start, end, DMA_BIDIRECTIONAL);
