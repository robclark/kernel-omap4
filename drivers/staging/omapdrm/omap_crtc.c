...
struct omap_crtc {
	...
	/* are we waiting for GO to clear? */
	bool waiting_go;
...
void omap_crtc_vblank(struct drm_crtc *crtc, uint32_t irqstatus)
{
	struct omap_crtc *omap_crtc = to_omap_crtc(crtc);

	if (omap_crtc->waiting_go &&
			!dispc_mgr_go_busy(omap_crtc->channel)) {
		struct omap_drm_private *priv =
				crtc->dev->dev_private;
		DBG("%s: apply done", omap_crtc->name);
		omap_crtc->waiting_go = false;
		drm_vblank_put(crtc->dev, omap_crtc->pipe);
...
static void apply_worker(struct work_struct *work)
{
	...
	if (need_apply) {
		...
		drm_vblank_get(dev, omap_crtc->pipe);
		omap_crtc->waiting_go = true;
